...
resDBDecoy <- createDecoyDB(files1,useContaminants = TRUE, annot=annotation,revLab = "DECOY_")
resDB <- createDecoyDB(files1,useContaminants = TRUE, annot=annotation,revLab = NULL)

xx <- file.path(x, paste(dbname,"_d_",format(Sys.time(), "%Y%M%d") ,sep=""))
writeFasta(resDBDecoy, file=xx)
xx <- file.path(x, paste(dbname,"_",format(Sys.time(), "%Y%M%d") ,sep=""))
